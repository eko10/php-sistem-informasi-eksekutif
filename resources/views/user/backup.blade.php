...
<div class="modal fade" id="ajaxModelChangePassword" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form id="changePasswordForm" name="changePasswordForm" class="form-horizontal">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h3 class="modal-title" id="modelHeadingChangePassword"></h3>
                </div>
                <div class="modal-body">
                    <span id="form_result_change_password"></span>
                    <div class="card-body card-block">
                        <input type="hidden" name="user_id" id="user_id_change">
                        <div class="form-group">
                            <label for="new_password" class="form-control-label">Password</label>
                            <input type="password" class="form-control" id="new_password" name="new_password" placeholder="Password">
                        </div>
                        <div class="form-group">
                            <label for="new_confirm_password" class="form-control-label">Konfirmasi Password</label>
                            <input type="password" class="form-control" id="new_confirm_password" name="new_confirm_password" placeholder="Password">
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
...
<script>
    $(document).ready(function () {
        ...
        $('body').on('click', '.changePassword', function () {
            ...
            $('#form_result_table').html('');
            $('#form_result_change_password').html('');
            $.get("{{ route('user.index') }}" + '/' + user_id + '/edit', function (data) {
                $('#modelHeadingChangePassword').html("Ubah Password");
                $('#updateBtn').val("change-password");
                $("#updateBtn").removeClass("btn-success");
                $("#updateBtn").addClass("btn-primary");
                $('#ajaxModelChangePassword').modal('show');
                $('#user_id_change').val(data.id);
            });
        });
        ...
        $('body').on('click', '.editUser', function () {
            ...
            $.get("{{ route('user.index') }}" + '/' + user_id + '/edit', function (data) {
                ...
                $("#role").data('select2').trigger('select', {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if(data.success) {
                        ...
                        $('#ajaxModelChangePassword').modal('hide');
                        $('#updateBtn').html('Simpan');
                        table.draw();
                    }
                    $('#form_result_change_password').html(html);
                    $('#updateBtn').html('Simpan');
                },
                error: function (data) {
                    console.log('Error:', data);
